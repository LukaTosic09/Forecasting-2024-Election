...
subtitle: "How Swing States Transform Elections Into Games of Chance"
author: 
  - Tara Chakkithara
  - Sakura Hu
  - Luka Tosic
...
date: today
date-format: long
abstract: "This paper uses the poll of polls method to develop a bayesian hierarcical model to predict the outcome of the 2024 U.S. election. Based on polling data collected up to October 19, 2024, our analysis forecasts that Kamala Harris is likely to win the election. Given the complexities of the electoral college system, this study specifically focuses on predicting the electoral college vote rather than the popular vote."
format: pdf
...
The United States has been the World's pinnacle of Hegemonic power for the last eighty to one hundered years, a nation-state powerful enough to exert its economic and military power as well as its societal and cultural influence on virtually all corners of the Globe. As much as many of our fellow Non-Americans loathe to hear it, the politics of the United States matter to people globally because their effect is felt globally. American citizens on the other hand have understand immediately the importance of the general election in influencing the direction their nation takes. 

The domestic political situation has escalated in the US for years, with higher rates of homelessness and income inequality being major points of concern. Moreover, alt right movements continue to gather steam targeting women's rights, minority rights and immigration. The current Democrat administration continues to be lambasted for its adherence to maintaining the status quo, with many openly protesting against the US's relationship with Israel and foreign involvement generally. We have also seen demonstration against the stripping away of reproductive rights, and against police brutality contrasted by anti-immigration marches.

The primary candidates in this election are the Democrat's Kamala Harris, and the Republican's Donald Trump. The estimand we will be using is a probability distribution of Trump's win percentage, which of course would inform Harris's in turn. Our estimate of this estimate will be found by analyzing the previous two elections, 2016's Clinton vs Trump and 2020's Trump vs Biden, as well as current polling data to make a prediction on either candidate's election rate and thus the winner of the election. Said estimate will allow us to make a prediction on who we think is likelier to win the election.

An important aspect of all elections, but particularly this one owing to the current volatile and unpredictable political climate, is the notion of so called "Swing" or "Battleground" states. In a presidential election, each state is awarded a number of electoral votes based on the state's total representation in congress, or more simply, its size. There are a total of 538 electoral votes, and a candidate needs 270 to secure a majority and the election. Some states have no consistent voting pattern over elections, these are the aforementioned swing states. We will focus our analysis on swing states in order to make an informed prediction about them and the election as a whole.

In particular, we used Bayesian updating and Monte Carlo Simulations of the election to model the results. We found this method of modeling was more effective than something like a Generalized Linear Model due to the nature of the variables found in the dataset. More information about the shape of the data is found in the data section, but in short we did not think that most of the variables present were capable creating a GLM that would accurately predict the results as none of the variables reasonably predict the hypothetical response of vote percentage based on our exploratory data analysis.

Our high level results are ...
...
# Data Overview (missing graphs and tables)
