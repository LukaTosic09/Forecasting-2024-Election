...
subtitle: "Analyzing Swing State Impact on 2024 Election Outcomes: Bayesian Hierarchical Model Using 538's Data Predicts 66% Harris Victory"
author: 
  - Tara Chakkithara
  - Sakura Hu
  - Luka Tosic
...
### Pollscore
The pollscore feature serves as a metric for evaluating the quality of a poll. It is a mesaurement that tries to quantify the realiablity of a poll. It is calculated based on factors such as poll transparency, bias, and error, @citeABC. A more negative pollscore indicates higher quality, reflecting a greater confidence in the poll's accuracy. As seen in @fig-pollscore, most polls collected by FiveThirtyEight have a pollscore under 0. In @fig-transparency, we see that lower poll scores are correlated with high transparency. 
...
### Sample Size
The sample size of a poll is the number of people who participated in the poll. Generally, a larger sample size is seen as a better representation of the population. The most common sample size of polls is around 1250, as seen in @fig-sample-size. Although this may seem like a small amount, sample sizes greater than 1004 people have a margin of error less than 3 percent @citeScientificAmerican. Standard political polls usually aim to include more than 1000 people. This is a good indicator that most of the polls in our data follow the accepted pratices of polling. 

The sample size of a poll refers to the number of individuals who participate in the survey. Generally, a larger sample size is considered to provide a more accurate representation of the overall population. For instance, a common sample size for polls is greater than 1000. While this might seem like a relatively small number, studies have shown that sample sizes exceeding 1,004 can achieve a margin of error of less than 3 percent (Scientific American). Sample sizes in our dataset have a mode of 1250, as seen in @fig-sample-size, which suggests that most of the polls in the dataset adhere to accepted polling practices when it comes to sample size. This commitment to robust sample sizes enhances the reliability of the polling data we analyze.
...
### Percentage of Votes
Each poll aims to . By examining how these values fluctuate over time and in response to different candidates, we can better understand their impact on public opinion. Currently, national level polls show Kamala Harris with a 46% approval rating compared to Donald Trump's 43%. This highlights the importance of candidate dynamics in shaping voter sentiment across the electorate.
...
#| fig-cap: "Voter Sentiment Before & After Biden's Candidacy Withdrawl"
data2 <- data %>%
  mutate(start_date = as.Date(sub("(..)$", "20\\1", start_date), format = "%m/%d/%Y")) %>%
  filter(start_date > biden_drop_date, pollscore < -1) %>%
